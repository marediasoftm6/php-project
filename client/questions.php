<div class="row g-4">
    <div class="col-12 col-lg-8">
            ...
            if (isset($_GET["c-id"])) {
                ...
                $stmt = $conn->prepare("select q.id, q.title, q.slug, u.id as user_id, u.username, count(a.id) as acnt from questions q left join answers a on a.question_id=q.id join users u on u.id=q.user_id where q.category_id=? group by q.id, q.title, q.slug, u.id, u.username");
                ...
                $stmt = $conn->prepare("select q.id, q.title, q.slug, u.id as user_id, u.username, count(a.id) as acnt from questions q left join answers a on a.question_id=q.id join users u on u.id=q.user_id where q.user_id=? group by q.id, q.title, q.slug, u.id, u.username");
                ...
                $like = "%" . $searchTerm . "%";
                $stmt = $conn->prepare("select q.id, q.title, q.slug, u.id as user_id, u.username, count(a.id) as acnt from questions q left join answers a on a.question_id=q.id join users u on u.id=q.user_id where q.title like ? group by q.id, q.title, q.slug, u.id, u.username");
                ...
                <div class="hero-section mb-5 p-5 rounded-4 text-center" style="background: var(--gradient); color: var(--white);">
                    ...
                    <?php if (!isset($_SESSION['user']['username'])) { ?>
                        <div class="d-flex justify-content-center gap-3 flex-wrap">
                            <a href="signup" class="btn btn-outline-light btn-lg px-4 rounded-pill">Get Started</a>
                            <a href="about" class="btn btn-light btn-lg px-4 rounded-pill text-primary">Learn More</a>
                        </div>
                    <?php } else { ?>
                        <div class="d-flex justify-content-center gap-3 flex-wrap">
                            <a href="ask-question" class="btn btn-outline-light btn-lg px-4 rounded-pill fw-regular">Ask a Question</a>
                            <a href="create-post" class="btn btn-light btn-lg px-4 rounded-pill text-primary fw-regular">Post Articles</a>
                        </div>
                    <?php } ?>
                </div>
