...
<div class="sidebar-card mt-4">
    ...
    <div class="d-flex flex-wrap gap-2">
        ...
        $query = "SELECT c.id, c.name, c.slug, COUNT(q.id) as q_count FROM category c LEFT JOIN questions q ON q.category_id = c.id GROUP BY c.id, c.name, c.slug ORDER BY q_count DESC LIMIT 15";
        $result = $conn->query($query);
        foreach($result as $row){
            $name = htmlspecialchars(ucfirst($row["name"]), ENT_QUOTES, 'UTF-8');
            $id = $row["id"];
            ?>
            <a href="<?php echo $row['slug']; ?>" class="badge-category text-decoration-none">
                <?php echo $name; ?>
            </a>
            <?php
        }?>
        ...
        ?>
            <div class="mb-3 pb-3 border-bottom border-light-subtle last-child-border-0">
                <div class="d-flex gap-3 align-items-start">
                    ...
                    <div>
                        <a href="<?php echo $p['slug']; ?>" class="text-decoration-none text-dark fw-bold small d-block mb-1 line-clamp-2">
...
<div class="sidebar-card mt-4">
    ...
    <a href="about" class="text-primary text-decoration-none small fw-bold">Learn more about us <i class="bi bi-arrow-right ms-1"></i></a>
</div>